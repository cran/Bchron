...
plot.BchronCalibratedDates = function(x,withPositions=FALSE,...) {
  ...
  if(length(x)==1) {
    ag = x[[1]]$ageGrid
    den = x[[1]]$densities
      plot(ag,den,type='l',main=names(x),...)      
  ...
  if(length(x)>1 & withPositions==FALSE) {
    for(i in 1:length(x)) {
      ag = x[[i]]$ageGrid
      den = x[[i]]$densities
      plot(ag,den,type='l',main=names(x)[i],...)
      mtext(paste(x[[i]]$calCurves),side=1,line=4,adj=0,cex=0.6)
      if(i<length(x)) readline('Hit Enter for next plot...')
    }
  }
  ...
  if(length(x)>1 & withPositions==TRUE) {
    ...
    # Create the plot
    plot(1,1,xlim=rev(xlimits),ylim=rev(ylimits),type="n",main='Calibrated dates by depth',...)
    for(i in 1:length(x)) {
